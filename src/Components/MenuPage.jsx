...
const MenuPage = () => {
  return (
    <div className="px-8 md:px-16 lg:px-24 py-12 min-h-screen bg-white flex flex-col items-center">
      <h1 className="text-4xl font-bold text-[#F17228] text-center">Our Special Menu</h1>
      <p className="mt-4 text-gray-600 text-center">Enjoy delicious meals crafted with perfection.</p>
        ...
        {menuItems.map((item) => (
          <div key={item.id} className="w-64">
            ...
            <div className="relative w-full h-64 overflow-hidden rounded-lg">
              <img src={item.img} alt={item.name} className="w-full h-full object-cover transition-transform transform hover:scale-105 duration-300" /> 
            </div>
           
            <div className="text-center mt-3">
              <h2 className="text-xl font-bold text-black">{item.name}</h2>
              <p className="text-gray-600 text-sm">{item.dis}</p>
              {/* <span className="text-lg font-bold text-black mt-2 block">{item.price}</span>   */}
            </div>
          </div>
        ))}
      </div>
